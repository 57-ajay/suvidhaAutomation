...
task ever fans out, so a failure on one challan can never affect another.
    ...
    requestId is the AUTOGENERATED doc id of the record we write. It is NOT the
    challan number — each challan has its own doc, but that doc is addressed by
    an opaque, case-sensitive Firestore auto-id:
