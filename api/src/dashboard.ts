...
<section id="view-run" class="hidden">
  ...
  <form class="runform hidden" id="form-challan-payment">
    ...
    <div class="field">
      <label>requestId (the subChallanRequests doc id)</label>
      <input type="text" id="cp-requestId" placeholder="gzvLTgBXlyVOcT1IOhNK">
      <div class="hint">the <b>autogenerated</b> Firestore doc id, copied from the real
        subChallanRequests record — <b>not</b> the challan number, and case matters.
        Inventing one creates a brand-new doc that no client reads.</div>
    </div>
    <div class="formrow">
      <div class="field">
    ...
    <div class="hint" style="margin-bottom:12px">
      One challan per run. requestId is the subChallanRequests doc id; the job id is <b>cp-&lt;requestId&gt;</b>.
...
function buildChallanPayment(){
  ...
  // requestId is the doc id and cannot be derived from anything — it has to be
  // the real autogenerated id. Deliberately NOT auto-filled by "Fill sample":
  // inventing one would silently write a fresh, orphaned doc instead of
  // failing loudly.
